...
fn test_a2_cross_file_instance_attr_dispatch() {
    ...
    // `self.backend.execute()` to backend.py::Backend.execute cross-file.
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
    std::fs::write(
    ...
    assert!(
        ...
        "A2: self.backend.execute() should resolve cross-file — Backend.execute.called_by should include Service.run. Got: {:?}",
        execute.called_by
    );
}
